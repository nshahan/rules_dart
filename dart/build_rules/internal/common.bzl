...
def make_dart_context(label,
  ...
  if not lib_root:
    # Should always be relative from the BUILD file location
    lib_root = "lib/"
...
def package_spec_action(ctx, dart_ctx, output):
  ...
  for dc in dart_ctxs:
    path_to_lib_root = dc.lib_root

    if dc.label.package.startswith("vendor/"):
      path_to_lib_root = "%s/%s" % (dc.label.package.replace("vendor/", "vendor_"), dc.lib_root)
    elif dc.label.workspace_root:
      path_to_lib_root = "%s/%s" % (dc.label.workspace_root, dc.lib_root)
    elif dc.label.package:
      path_to_lib_root = "%s/%s" % (dc.label.package, dc.lib_root)
